...
// The interactive journey portal. Rendered inside the shared <SitePage> (which supplies the
// signed-in header + footer), so the logged-in identity is consistent with every other page.
export function JourneyView({ user }: { user: PublicUser }) {
  ...
  return (
    <>
      ...
      <div className="mx-auto w-full max-w-5xl p-[clamp(16px,3vw,30px)]">
        {/* booking hero */}
        <div className="relative mb-5 overflow-hidden rounded-[18px] bg-gradient-to-br from-green-800 to-green-900 p-[clamp(22px,2.6vw,30px)] text-green-50">
          <div className="mb-3 text-[13.5px] font-medium text-green-100/85">
            Assalamu alaikum, <span className="font-semibold text-white">{firstName}</span> — here is your journey.
          </div>
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_1.4fr]">
      <Panel title="Summary">
        ...
        <button type="button" className="mt-4 w-full rounded-xl bg-green-800 py-2.5 text-sm font-semibold text-white transition-[transform,background-color] duration-fast hover:bg-green-700 active:scale-[0.98]">Pay balance</button>
        <button type="button" className="mt-2 w-full rounded-xl border-[1.5px] border-sand-300 bg-white py-2.5 text-sm font-semibold text-sand-700 transition-colors duration-fast hover:bg-sand-100">Download receipt / plan</button>
      </Panel>
