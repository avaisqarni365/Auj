...
const PIPELINE = [
  { label: 'e-Visa · submitted', count: 18, pct: '70%', color: 'bg-success' },
  { label: 'Agent channel · review', count: 9, pct: '40%', color: 'bg-info' },
  { label: 'Issued', count: 24, pct: '88%', color: 'bg-green-800' },
];

export function AgentDashboard({ agent, walletBalance, available, bookings }: AgentDashboardProps) {
  const kpis = [
    { label: 'Wallet balance', value: formatMoney(walletBalance) },
    ...
    <div className="grid gap-4">
      ...
      <div className="grid gap-3.5 md:grid-cols-[1.6fr_1fr]">
        <div className="rounded-2xl border border-sand-200 bg-white p-5">
          <div className="mb-3.5 text-sm font-bold">Visa pipeline</div>
          {PIPELINE.map((v) => (
        ...
        <div className="rounded-2xl bg-green-800 p-5 text-green-50">
          <div className="text-[13px] text-green-100/80">Credit available</div>
