...
/** Pull live Makkah + Madinah offers and pair them into 3 deal cards (cheapest → premium). */
export async function buildDeals(): Promise<Deal[]> {
  try {
    ...
    return plans.map((p, i) => {
      ...
      const price =
        m.nightlyNet.amount * p.makkahNights + d.nightlyNet.amount * p.madinahNights + FLIGHT_ESTIMATE;
      return {
        id: `${m.id}-${d.id}`,
