    ...
    <div className="min-h-screen bg-sand-50">
      <header className="border-b border-sand-200 bg-white">
        <div className="mx-auto flex max-w-3xl items-center justify-between px-4 py-3">
          ...
          <span className="text-[13px] text-sand-500">Support · {user.email}</span>
        </div>
      </header>

      <main className="mx-auto max-w-3xl px-4 py-6">
        <h1 className="mb-1 font-serif text-2xl font-semibold">Help &amp; support</h1>
        <p className="mb-5 text-sm text-sand-500">Open a request or continue a conversation. We reply here and by email.</p>

        <OpenTicketForm />

        <div className="mt-7 mb-2.5 text-[13px] font-bold">Your tickets</div>
        {tickets.length === 0 ? (
          <p className="rounded-2xl border border-dashed border-sand-300 bg-white p-6 text-center text-sm text-sand-500">No tickets yet.</p>
        ) : (
...
function TicketCard({ ticket }: { ticket: Ticket }) {
  ...
  return (
    <div className="rounded-2xl border border-sand-200 bg-white p-4">
      ...
      {closed ? (
        <p className="mt-3 text-[12px] text-sand-500">This ticket is {ticket.status.toLowerCase()}. Open a new request if you need more help.</p>
      ) : (
        <form action={replyTicketAction} className="mt-3 flex gap-2">
          <input type="hidden" name="ticketId" value={ticket.id} />
          <input name="body" required placeholder="Write a reply…" className="flex-1 rounded-[10px] border-[1.5px] border-sand-300 px-3 py-2 text-[14px] focus:border-green-700 focus:outline-none" />
          <button type="submit" className="rounded-[10px] bg-green-800 px-4 py-2 text-sm font-semibold text-white hover:bg-green-700">Reply</button>
        </form>
      )}
    </div>
  );
